...
const SkillIcon = ({ name, size = 35 }: { name: string; size?: number }) => {
  return (
    <Tooltip>
      ...
      <TooltipPortal>
        <TooltipContent side="bottom" align="center" sideOffset={8} className="text-xs font-medium">
          {formatTechName(name)}
        </TooltipContent>
      </TooltipPortal>
    </Tooltip>
...
const projects: ProjectCardProps[] = [
  ...
  },
  {
    ...
    technologies: [<SkillIcon name="nodejs" key="nodejs" />, <SkillIcon name="bash" key="bash" />],
  ...
  },
  {
    ...
    technologies: [<SkillIcon name="deno" key="deno" />, <SkillIcon name="ts" key="typescript" />],
...
export default function Projects() {
  ...
  return (
    <motion.section
      className="relative mx-10 flex h-full min-h-screen w-full min-w-screen flex-col gap-10 md:p-10"
        ...
        <div className="mb-8 flex flex-wrap items-center justify-center gap-4">
          {allCategories.map((category) => (
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className={
                "rounded-xl bg-gray-200 px-4 py-2 text-xs font-medium text-gray-800 hover:bg-gray-300 data-[active=true]:bg-purple-600 data-[active=true]:text-white dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600"
              }
            >
              {category}
            </motion.button>
